...
function createCategory(nom) {
    let html = `
        <li class="mb-1">
            <button class="btn btn-toggle align-items-center rounded collapsed float-right" data-bs-toggle="collapse" data-bs-target="#beaute-collapse" aria-expanded="true">
                ${nom}
            </button>
        </li>
        `
...
function displayNav() {
    ...
    let categories = loadResource("/api/categories").then(categories => {
        ...
        categories.forEach(element => {
            ...
            let productsHTML = "";
            productsHTML += `
            <li><div draggable="true">Item</div><a type="button" data-bs-toggle="modal" data-bs-target="#exampleModal"><span class="badge bg-info float-right">i</span></a></li>`
            html += productsHTML
